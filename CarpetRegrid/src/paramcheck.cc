...
void CarpetRegridParamcheck(CCTK_ARGUMENTS) {
  ...
  if (refinement_levels > maxreflevels) {
    CCTK_PARAMWARN("The parameter CarpetRegrid::refinement_levels is larger "
                   "than Carpet::max_refinement_levels");
  }
  ...
  if (smart_outer_boundaries) {
    ...
    if (!*domain_from_coordbase) {
      CCTK_PARAMWARN("The parameter CarpetRegrid::smart_outer_boundaries can "
                     "only be used when Carpet::domain_from_coordbase=yes");
    }
    if (CCTK_Equals(refined_regions, "manual-coordinate-list")) {
      // do nothing
    } else {
      CCTK_PARAMWARN("The parameter CarpetRegrid::smart_outer_boundaries can "
                     "currently only be used when "
                     "CarpetRegrid::refined_regions is set to "
