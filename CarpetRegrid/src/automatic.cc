...
int Automatic(cGH const *const cctkGH, gh const &hh, gh::rregs &regss) {
  ...
  Automatic_OneLevel(cctkGH, hh, reflevel,
                     min(reflevels + 1, (int)refinement_levels), errorgf, regs);
...
void Automatic_OneLevel(const cGH *const cctkGH, const gh &hh, const int rl,
                        const int numrl, const gf<CCTK_REAL> &errorgf,
                        vector<region_t> &regs) {
  DECLARE_CCTK_PARAMETERS;
  ...
  if (veryverbose) {
    cout << endl
         << "MRA: Choosing regions to refine on level " << rl << " in "
         << hh.components(rl) << " components" << endl;
  }
  ...
  if (veryverbose) {
    int numpoints = 0;
    for (list<region_t>::const_iterator ireg = regl.begin(); ireg != regl.end();
         ++ireg) {
      numpoints += ireg->extent.size();
    }
    cout << "MRA: Chose " << regl.size() << " regions with a total size of "
         << numpoints << " to refine." << endl
         << endl;
  }
  ...
  for (list<region_t>::const_iterator it = regl.begin(); it != regl.end();
       ++it) {
    regs.push_back(*it);
  }
  ...
  for (size_t c = 0; c < regs.size(); ++c) {
    const ivect lb = either(
        ...
        max(regs.at(c).extent.lower(), hh.baseextents.at(0).at(0).lower()));
    const ivect ub = either(
        obp[1], regs.at(c).extent.upper(),
        min(regs.at(c).extent.upper(), hh.baseextents.at(0).at(rl).upper()));
...
void Automatic_Recursive(const cGH *const cctkGH, const gh &hh,
  ...
  if (cnt == 0) {
    // Don't refine
  } else if (any(reffact * width < (int)(2 * minwidth)) or
    ...
    if (veryverbose) {
      cout << "MRA: Refining to " << region0.extent << " size "
           << region0.extent.size() << " fraction " << fraction << endl;
    }
  } else {
...

      {
        ...
        while (ireg2 != regl2.end()) {
          ...
          if (iface1 == iface2) {
            ...
            combined.extent =
                ibbox(ireg1->extent.lower(), ireg2->extent.upper(), str);
            ...
            if (veryverbose) {
              assert(dir < 3);
              cout << "MRA: Combining along "
                   << "xyz"[dir] << " to " << regl.back().extent << " size "
                   << regl.back().extent.size() << endl;
            }
