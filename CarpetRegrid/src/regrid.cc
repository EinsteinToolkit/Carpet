...
CCTK_INT CarpetRegrid_Regrid(CCTK_POINTER_TO_CONST const cctkGH_,
                             ...
                             CCTK_POINTER const regsss_, CCTK_INT force) {
  ...
  if (!force) {
    ...
    assert(regrid_every == -1 or regrid_every == 0 or
    ...
    if ((regrid_every > 0 and cctkGH->cctk_iteration != 0 and
         (cctkGH->cctk_iteration - 1) % regrid_every != 0) or
    ...
    if (CCTK_EQUALS(activate_levels_on_regrid, "none")) {
      ...
      if (cctkGH->cctk_iteration - 1 >= activate_next) {
        ...
        *const_cast<CCTK_INT *>(&activate_next) = cctkGH->cctk_iteration;
        ostringstream next;
        next << activate_next;
        CCTK_ParameterSet("activate_next", "CarpetRegrid", next.str().c_str());

        *const_cast<CCTK_INT *>(&refinement_levels) = newnumlevels;
        ostringstream param;
        param << refinement_levels;
        CCTK_ParameterSet("refinement_levels", "CarpetRegrid",
                          param.str().c_str());

        if (verbose) {
          ostringstream buf1, buf2;
          buf1 << "Activating " << newnumlevels - oldnumlevels
               << " new refinement levels";
      ...
      if (newnumlevels > 0 and newnumlevels <= maxreflevels) {
        ...
        *const_cast<CCTK_INT *>(&refinement_levels) = newnumlevels;
        ...
        CCTK_VWarn(1, __LINE__, __FILE__, CCTK_THORNSTRING,
                   "The aliased function \"RegridLevel\" returned an illegal "
                   "number of refinement levels (%d).  No levels will be "
  ...
  if (CCTK_EQUALS(refined_regions, "none")) {
    ...
    do_recompose = ManualGridpoints(cctkGH, hh, superregss);

  } else if (CCTK_EQUALS(refined_regions, "manual-coordinates")) {

    do_recompose = ManualCoordinates(cctkGH, hh, superregss);

  } else if (CCTK_EQUALS(refined_regions, "manual-gridpoint-list")) {

    do_recompose = ManualGridpointList(cctkGH, hh, superregss);

  } else if (CCTK_EQUALS(refined_regions, "manual-coordinate-list")) {

    do_recompose = ManualCoordinateList(cctkGH, hh, superregss);
