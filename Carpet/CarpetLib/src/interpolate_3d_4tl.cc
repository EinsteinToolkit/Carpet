...
#define DSTIND3(i,j,k)                                  \
                      ...
                      ibbox3 const & restrict regbbox)
  {
    ...
    RT const s1fac = (t - t2) * (t - t3) * (t - t4) / ((t1 - t2) * (t1 - t3) * (t1 - t4));
    RT const s2fac = (t - t1) * (t - t3) * (t - t4) / ((t2 - t1) * (t2 - t3) * (t2 - t4));
    RT const s3fac = (t - t1) * (t - t2) * (t - t4) / ((t3 - t1) * (t3 - t2) * (t3 - t4));
    RT const s4fac = (t - t1) * (t - t2) * (t - t3) / ((t4 - t1) * (t4 - t2) * (t4 - t3));
