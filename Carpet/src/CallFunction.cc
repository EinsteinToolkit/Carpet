                 ...
                 void *data) ///< private data for CCTK_CallFunction
{
  ...
  bool const do_psync = not (CCTK_EQUALS(presync_mode, "off") or
  ...
  if(not CCTK_EQUALS(presync_mode, "presync-only")) {
    ...
    for (int g = 0; g < attribute->n_SyncGroups; g++) {
      ...
      if (nvar > 0) {
        ...
        bool do_sync_group = true;
        if (do_psync) {
          // to support mixed PreSync / NonPreSync runs, skip all SYNC request
          // for groups that have Driver BC associated with them and are thus
          // SYNCed automatically
          int const n0 = CCTK_FirstVarIndexI(group);
          assert(n0 >= 0);
          for (int var = 0; var < nvar; ++var) {
            assert(n0 + var < CCTK_NumVars());
            if (QueryDriverBCForVarI(cctkGH, n0 + var)) {
              do_sync_group = false;
              break;
            }
          }
  ...
  // groups to SYNC due to READS
  std::vector<int> pre_groups;
  if(use_psync)
    PreCheckValid(attribute, cctkGH, pre_groups);
  ...
  if(use_psync)
    PostCheckValid(attribute, cctkGH);
