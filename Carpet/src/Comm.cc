                                         ...
                                         const vector<int>& groups)
  {
    ...
    BEGIN_LEVEL_MODE(cctkGH) {
      ...
      BEGIN_MAP_LOOP(cctkGH, CCTK_GF) {
        ...
        for (size_t ip = 0 ;
             ip < fast_boxes.fast_ref_bnd_prol_sendrecv.size() ;
             ++ip) {
...
#pragma omp parallel for
            for (size_t v=0; v<vis.size(); ++v) {
              ...
              char *vardataptr = static_cast<char*>(cctkGH->data[vi][0 /*tl*/]); // DECLARE_CCTK_ARGUMENTS always puts the correct pointer into tl=0
              if (vardataptr == NULL) {
                int const active_tl =
                  groupdata.AT(CCTK_GroupIndexFromVarI(vi)).activetimelevels.AT(mglevel).AT(reflevel);
#pragma omp critical
                CCTK_VWarn(CCTK_WARN_ABORT, __LINE__, __FILE__, CCTK_THORNSTRING,
                           "Got NULL vardataptr for '%s' tl %d vi %d timelevel %d atl %d",
                           CCTK_VarName(vi), tl, vi, timelevel, active_tl);
              }
              assert (vardataptr);
              LOOP_OVER_BBOX(cctkGH, rp.extent, box, imin, imax) {
#pragma omp parallel
                CCTK_LOOP3(poison_boundary,
                           i,j,k,
                           imin[0],imin[1],imin[2], imin[0]+1,imax[1],imax[2],
