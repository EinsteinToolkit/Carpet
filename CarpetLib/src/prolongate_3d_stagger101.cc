    ...
    for (int ii = 1; ii <= ORDER + 1; ii++)
      for (int jj = 1; jj <= ORDER + 1; jj++) {
        dst[DSTIND3(id, jd, kd)] +=
            ...
            src[SRCIND3(is - 1 + (ii - 1), js, ks - 1 + (jj - 1))];
      }
  }
  if (ORDER == 3) {
    for (int ii = 1; ii <= ORDER + 1; ii++)
      for (int jj = 1; jj <= ORDER; jj++)
        for (int kk = 1; kk <= ORDER + 1; kk++) {
          dst[DSTIND3(id, jd, kd)] +=
              coeff[1][ii] * coeff[4][jj] * coeff[1][kk] *
              src[SRCIND3(is - 2 + (ii - 1), js - 1 + (jj - 1),
                          ks - 2 + (kk - 1))];
        }
  }
  if (ORDER == 4) {
  ...
  if (ORDER == 3) {
    for (int ii = 1; ii <= ORDER + 1; ii++)
      for (int jj = 1; jj <= ORDER; jj++)
        for (int kk = 1; kk <= ORDER + 1; kk++) {
          dst[DSTIND3(id, jd, kd)] +=
              coeff[2][ii] * coeff[4][jj] * coeff[1][kk] *
              src[SRCIND3(is - 2 + (ii - 1) + (4 - ORDER), js - 1 + (jj - 1),
                          ks - 2 + (kk - 1))];
        }
  }
  if (ORDER == 4) {
  ...
  if (ORDER <= 2) {
    for (int ii = 1; ii <= ORDER + 1; ii++)
      for (int jj = 1; jj <= ORDER + 1; jj++)
        for (int kk = 1; kk <= ORDER + 1; kk++) {
          dst[DSTIND3(id, jd, kd)] +=
              coeff[1][ii] * coeff[3][jj] * coeff[1][kk] *
  ...
  if (ORDER <= 2) {
    for (int ii = 1; ii <= ORDER + 1; ii++)
      for (int jj = 1; jj <= ORDER + 1; jj++)
        for (int kk = 1; kk <= ORDER + 1; kk++) {
          dst[DSTIND3(id, jd, kd)] +=
              coeff[2][ii] * coeff[3][jj] * coeff[1][kk] *
              src[SRCIND3(is - 1 + (ii - 1), js - 1 + (jj - 1),
                          ks - 1 + (kk - 1))];
        }
  }
  ...
  if (ORDER == 3) {
    for (int ii = 1; ii <= ORDER + 1; ii++)
      for (int jj = 1; jj <= ORDER; jj++)
        for (int kk = 1; kk <= ORDER + 1; kk++) {
          dst[DSTIND3(id, jd, kd)] +=
              coeff[1][ii] * coeff[4][jj] * coeff[2][kk] *
              src[SRCIND3(is - 2 + (ii - 1), js - 1 + (jj - 1),
                          ks - 2 + (kk - 1) + (4 - ORDER))];
        }
  }
  ...
  if (ORDER == 3) {
    for (int ii = 1; ii <= ORDER + 1; ii++)
      for (int jj = 1; jj <= ORDER; jj++)
        for (int kk = 1; kk <= ORDER + 1; kk++) {
          dst[DSTIND3(id, jd, kd)] +=
              coeff[2][ii] * coeff[4][jj] * coeff[2][kk] *
              src[SRCIND3(is - 2 + (ii - 1) + (4 - ORDER), js - 1 + (jj - 1),
                          ks - 2 + (kk - 1) + (4 - ORDER))];
        }
  }
  ...
  if (ORDER <= 2) {
    for (int ii = 1; ii <= ORDER + 1; ii++)
      for (int jj = 1; jj <= ORDER + 1; jj++)
        for (int kk = 1; kk <= ORDER + 1; kk++) {
          dst[DSTIND3(id, jd, kd)] +=
              coeff[1][ii] * coeff[3][jj] * coeff[2][kk] *
              src[SRCIND3(is - 1 + (ii - 1), js - 1 + (jj - 1),
                          ks - 1 + (kk - 1))];
        }
  }
  ...
  if (ORDER <= 2) {
    for (int ii = 1; ii <= ORDER + 1; ii++)
      for (int jj = 1; jj <= ORDER + 1; jj++)
        for (int kk = 1; kk <= ORDER + 1; kk++) {
          dst[DSTIND3(id, jd, kd)] +=
              coeff[2][ii] * coeff[3][jj] * coeff[2][kk] *
              src[SRCIND3(is - 1 + (ii - 1), js - 1 + (jj - 1),
                          ks - 1 + (kk - 1))];
        }
  }
...
#define TYPECASE(N, T)                                                         \
                                                                               \
  template void prolongate_3d_stagger101<T, 2>(                                \
      T const *restrict const src, ivect3 const &restrict srcpadext,           \
      ivect3 const &restrict srcext, T *restrict const dst,                    \
      ivect3 const &restrict dstpadext, ivect3 const &restrict dstext,         \
      ibbox3 const &restrict srcbbox, ibbox3 const &restrict dstbbox,          \
      ...
      void *extraargs);                                                        \
                                                                               \
  template void prolongate_3d_stagger101<T, 3>(                                \
      T const *restrict const src, ivect3 const &restrict srcpadext,           \
      ivect3 const &restrict srcext, T *restrict const dst,                    \
      ivect3 const &restrict dstpadext, ivect3 const &restrict dstext,         \
      ibbox3 const &restrict srcbbox, ibbox3 const &restrict dstbbox,          \
      ...
      void *extraargs);                                                        \
                                                                               \
  template void prolongate_3d_stagger101<T, 4>(                                \
      T const *restrict const src, ivect3 const &restrict srcpadext,           \
      ivect3 const &restrict srcext, T *restrict const dst,                    \
      ivect3 const &restrict dstpadext, ivect3 const &restrict dstext,         \
      ibbox3 const &restrict srcbbox, ibbox3 const &restrict dstbbox,          \
