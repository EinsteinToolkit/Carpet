...
l8000:
  ...
  // in all directions:
  // Interpolate to (is-1/4, js-1/4, ks-1/4), where is,js,ks is a point on the
  // COARSE grid, since our points are all cell-centered
      ...
      for (int jj = 1; jj <= ORDER + 1; jj++)
        for (int kk = 1; kk <= ORDER + 1; kk++) {
          dst[DSTIND3(id, jd, kd)] +=
              coeff[1][ii] * coeff[1][jj] * coeff[1][kk] *
  ...
  if (ORDER <= 2) {
    for (int ii = 1; ii <= ORDER + 1; ii++)
      for (int jj = 1; jj <= ORDER + 1; jj++)
        for (int kk = 1; kk <= ORDER + 1; kk++) {
          dst[DSTIND3(id, jd, kd)] +=
              coeff[2][ii] * coeff[1][jj] * coeff[1][kk] *
              src[SRCIND3(is - 1 + (ii - 1), js - 1 + (jj - 1),
                          ks - 1 + (kk - 1))];
        }
  }
  ...
  if (ORDER <= 2) {
    for (int ii = 1; ii <= ORDER + 1; ii++)
      for (int jj = 1; jj <= ORDER + 1; jj++)
        for (int kk = 1; kk <= ORDER + 1; kk++) {
          dst[DSTIND3(id, jd, kd)] +=
              coeff[1][ii] * coeff[2][jj] * coeff[1][kk] *
              src[SRCIND3(is - 1 + (ii - 1), js - 1 + (jj - 1),
                          ks - 1 + (kk - 1))];
        }
  }
  ...
  if (ORDER <= 2) {
    for (int ii = 1; ii <= ORDER + 1; ii++)
      for (int jj = 1; jj <= ORDER + 1; jj++)
        for (int kk = 1; kk <= ORDER + 1; kk++) {
          dst[DSTIND3(id, jd, kd)] +=
              coeff[2][ii] * coeff[2][jj] * coeff[1][kk] *
              src[SRCIND3(is - 1 + (ii - 1), js - 1 + (jj - 1),
                          ks - 1 + (kk - 1))];
        }
  }
  ...
  if (ORDER <= 2) {
    for (int ii = 1; ii <= ORDER + 1; ii++)
      for (int jj = 1; jj <= ORDER + 1; jj++)
        for (int kk = 1; kk <= ORDER + 1; kk++) {
          dst[DSTIND3(id, jd, kd)] +=
              coeff[1][ii] * coeff[1][jj] * coeff[2][kk] *
              src[SRCIND3(is - 1 + (ii - 1), js - 1 + (jj - 1),
                          ks - 1 + (kk - 1))];
        }
  }
  ...
  if (ORDER <= 2) {
    for (int ii = 1; ii <= ORDER + 1; ii++)
      for (int jj = 1; jj <= ORDER + 1; jj++)
        for (int kk = 1; kk <= ORDER + 1; kk++) {
          dst[DSTIND3(id, jd, kd)] +=
              coeff[2][ii] * coeff[1][jj] * coeff[2][kk] *
              src[SRCIND3(is - 1 + (ii - 1), js - 1 + (jj - 1),
                          ks - 1 + (kk - 1))];
        }
  }
  ...
  if (ORDER <= 2) {
    for (int ii = 1; ii <= ORDER + 1; ii++)
      for (int jj = 1; jj <= ORDER + 1; jj++)
        for (int kk = 1; kk <= ORDER + 1; kk++) {
          dst[DSTIND3(id, jd, kd)] +=
              coeff[1][ii] * coeff[2][jj] * coeff[2][kk] *
              src[SRCIND3(is - 1 + (ii - 1), js - 1 + (jj - 1),
                          ks - 1 + (kk - 1))];
        }
  }
  ...
  if (ORDER <= 2) {
    for (int ii = 1; ii <= ORDER + 1; ii++)
      for (int jj = 1; jj <= ORDER + 1; jj++)
        for (int kk = 1; kk <= ORDER + 1; kk++) {
          dst[DSTIND3(id, jd, kd)] +=
              coeff[2][ii] * coeff[2][jj] * coeff[2][kk] *
              src[SRCIND3(is - 1 + (ii - 1), js - 1 + (jj - 1),
                          ks - 1 + (kk - 1))];
        }
  }
...
#define TYPECASE(N, T)                                                         \
                                                                               \
  template void prolongate_3d_stagger111<T, 2>(                                \
      T const *restrict const src, ivect3 const &restrict srcpadext,           \
      ivect3 const &restrict srcext, T *restrict const dst,                    \
      ivect3 const &restrict dstpadext, ivect3 const &restrict dstext,         \
      ibbox3 const &restrict srcbbox, ibbox3 const &restrict dstbbox,          \
      ...
      void *extraargs);                                                        \
                                                                               \
  template void prolongate_3d_stagger111<T, 3>(                                \
      T const *restrict const src, ivect3 const &restrict srcpadext,           \
      ivect3 const &restrict srcext, T *restrict const dst,                    \
      ivect3 const &restrict dstpadext, ivect3 const &restrict dstext,         \
      ibbox3 const &restrict srcbbox, ibbox3 const &restrict dstbbox,          \
      ...
      void *extraargs);                                                        \
                                                                               \
  template void prolongate_3d_stagger111<T, 4>(                                \
      T const *restrict const src, ivect3 const &restrict srcpadext,           \
      ivect3 const &restrict srcext, T *restrict const dst,                    \
      ivect3 const &restrict dstpadext, ivect3 const &restrict dstext,         \
      ibbox3 const &restrict srcbbox, ibbox3 const &restrict dstbbox,          \
