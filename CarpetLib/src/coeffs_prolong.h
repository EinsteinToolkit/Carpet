    ...
    // Here we set coefficients for interpolation to point i-1/4 (staggered coarse grid _array index_ i), using information i-2,i-1,i,i+1.
    ...
    // Interpolation coefficients are set by solving the following equation for (a,b,c,d)=(f11,f12,f13,f14):
    // a f(x1) + b f(x2) + c f(x3) + d f(x4) = f(x5), where (x1,x2,x3,x4,x5)=(-2,-1,0,1,-1/4).
    // If f(x) = k = constant, we have
    // a + b + c + d = 1
    // If f(x) = k x, we have
    // a x1 + b x2 + c x3 + d x4 = x5
    // Similarly for f(x) = k x^2 and f(x) = k x^3, so we have a cubic polynomial fit
    ...
    // Here we set coefficients for interpolation to point i+1/4 (staggered coarse grid _array index_ i), using information i-1,i,i+1,i+2
    ...
    // Here we set coefficients for interpolation to point i+1/2 (unstaggered coarse grid _array index_ i), using information i-1,i,i+1,i+2
    ...
    // Here we set coefficients for interpolation to point i (unstaggered coarse grid _array index_ i), using information i-1,i,i+1
