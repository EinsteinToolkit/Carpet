...
static void slurp_symtab(bfd *abfd) {
  ...
  if (symcount == 0)
    symcount =
        bfd_read_minisymbols(abfd, true /* dynamic */, (PTR)&syms, &size);
...
static void find_address_in_section(bfd *abfd, asection *section,
  ...
  found = bfd_find_nearest_line(abfd, section, syms, pc - vma, &filename,
                                &functionname, &line);
}
...
static int find_matching_file(struct dl_phdr_info *info, size_t size,
                              void *data) {
