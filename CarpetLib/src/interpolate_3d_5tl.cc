...
void interpolate_3d_5tl(T const *restrict const src1, CCTK_REAL const t1,
  ...
  RT const s2fac = (t - t1) * (t - t3) * (t - t4) * (t - t5) /
                   ((t2 - t1) * (t2 - t3) * (t2 - t4) * (t2 - t5));
  RT const s3fac = (t - t1) * (t - t2) * (t - t4) * (t - t5) /
                   ((t3 - t1) * (t3 - t2) * (t3 - t4) * (t3 - t5));
  RT const s4fac = (t - t1) * (t - t2) * (t - t3) * (t - t5) /
...
#define TYPECASE(N, T)                                                         \
  template void interpolate_3d_5tl(                                            \
